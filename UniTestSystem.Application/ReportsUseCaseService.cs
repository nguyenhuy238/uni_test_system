...
public sealed class ReportsUseCaseService : IReportsUseCaseService
{
    ...
    {
        var facultyVm = await _reportService.GetFacultyReportAsync(fromUtc, toUtc);
        var yearVm = await _reportService.GetAcademicYearReportAsync(fromUtc, toUtc);
        var dashboardVm = await _reportService.GetWidgetDashboardAsync(fromUtc, toUtc, actorRole, actorUserId);
    ...
    {
        return _reportService.GetQuestionAnalyticsAsync(fromUtc, toUtc, courseId, minAttempts);
    }

    public Task<LecturerPerformanceVm> GetLecturerPerformanceVmAsync(DateTime fromUtc, DateTime toUtc, string? lecturerId)
    {
        return _reportService.GetLecturerPerformanceReportAsync(fromUtc, toUtc, lecturerId);
    }

    public Task<StudentSubjectReportVm> GetStudentSubjectVmAsync(string userId, DateTime fromUtc, DateTime toUtc)
    {
        return _reportService.GetStudentSubjectReportAsync(userId, fromUtc, toUtc);
    }
    ...
    {
        var vm = await _reportService.GetStudentSubjectReportAsync(userId, fromUtc, toUtc);
